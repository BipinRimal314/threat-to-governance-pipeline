...
% ============================================================================
\begin{abstract}
AI agent monitoring and insider threat detection share the same architecture: profile a baseline, flag deviations, encode assumptions about trust. We make this concrete across eleven experiments. Using a Unified Behavioural Feature Schema (UBFS) that maps both employee activity logs and agent execution traces into a shared representation, we apply three anomaly detection models---Isolation Forest, LSTM Autoencoder, and Deep Clustering---across five domains. Cross-domain transfer works: an Isolation Forest trained on 329,000 insider threat user-days retains \textbf{97\% of detection power} on agent traces, and transfer to MCP tool-calling benchmarks exceeds within-domain performance (104.8\% retention). But the blind spots transfer too. Model distillation attacks span a detection spectrum from trivially caught (FOCUSED extraction: 0.99 AUC-ROC) to near-chance (HYDRA distributed: 0.54), and task decomposition---splitting a malicious objective into innocent subtasks---costs 5--6\% detection power across all models. The most persistent blind spot is Tool Misuse (ASI02), where agents use the right tools in the right order with escalated privileges. Augmenting UBFS with eight semantic features (UBFS-28) dramatically improves detection for four of five OWASP categories but \textbf{fails to close ASI02} (${\sim}$0.56 AUC-ROC). Adversarial evasion testing reveals that mimicry attacks---copying a normal neighbour's features while preserving the attack payload---degrade detection by up to 25\%, with Memory Poisoning (ASI05) most vulnerable. Temporal window ablation finds that 10 spans are optimal for Isolation Forest on both TRAIL and ATBench, mirroring the 7-day window finding from insider threat detection. Mapping five MITRE ATLAS techniques~\cite{mitre2024atlas} into the UBFS confirms that the framework generalises beyond OWASP: all five techniques are strongly detectable ($>$0.93 AUC-ROC for IF/LSTM), introducing no new blind spots. The detection models port across domains. So do their biases, and so do their blind spots.
\end{abstract}
...
\textbf{Experiment~3} (OWASP detection matrix): Evaluate IF and DC on synthetic OWASP anomalies injected into TRAIL traces.\footnote{Experiments 3 and 7 report IF and DC only. The LSTM Autoencoder operates on pseudo-sequences of length 1 for these point-in-time synthetic features, providing no temporal modelling advantage; Experiments 5, 6, and 8--11 include all three models.} Maps detections to the OWASP Top~10 for Agentic Applications. [\RQ{2}]
...
\textbf{Experiment~7} (MCP transfer): Extend cross-domain transfer to ATBench MCP tool-calling traces (IF and DC), testing whether the UBFS bridge generalises beyond the original TRAIL/TRACE/CERT datasets. [\RQ{1}, \RQ{2}]
...
Decomposition costs 5--6\% detection power for IF and DC (std $<$0.02 across 5 seeds), with LSTM showing near-invariance ($-0.007$). The IF and DC drops exceed their respective within-seed standard deviations, indicating a consistent effect rather than noise. The pattern mirrors insider threat evasion: in~\cite{rimal2026insider}, gradual escalation (Scenario~2) dropped detection from 69\% to 3\%. The agent-domain effect is smaller in absolute terms because the direct attacks are already harder to detect (0.72--0.77 vs.\ Scenario~1's 0.69 recall). But the mechanism is identical---spreading malicious activity across multiple steps reduces each step's anomaly score below the detection threshold.
...
\textbf{Third, better features do not close the intent blind spot.} Augmenting UBFS with 8 semantic dimensions (Experiment~8) dramatically improves detection for four OWASP categories---LSTM on ASI01 jumps from 0.63 to 0.94 ($+0.31$)---but leaves ASI02 at ${\sim}$0.56. The missing signal is authorisation context, not intent features. Closing ASI02 requires architectural changes to how agents are authorised, not better monitoring.
...
\textbf{Seventh, the framework generalises beyond OWASP.} All five MITRE ATLAS techniques are strongly detectable through UBFS ($>$0.93 AUC-ROC for IF/LSTM, Experiment~11). No new blind spots emerge. The ATLAS techniques---model replication, extraction, adversarial crafting---have strong volume and frequency signatures that UBFS captures by design. The only blind spot remains intent-based: attacks that use the right tools for the wrong reasons.
