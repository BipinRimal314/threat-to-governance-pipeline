...
\midrule
ASI06 Mem.\ \& Ctx.\ Poisoning & $.920{\pm}.010$ & $\mathbf{.975{\pm}.006}$ & $.981{\pm}.013$ \\
...
\bottomrule
\multicolumn{4}{l}{\footnotesize $^{\dagger}$Not an ASI category; see note below.} \\
\end{tabular}
\end{table}
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^{\dagger}$OWASP LLM Top 10; no ASI equivalent (Section~\ref{sec:owasp}).} \\
\end{tabular}
\end{table}
...
\toprule
\textbf{Strategy} & \textbf{Model} & \textbf{ASI01} & \textbf{ASI02} & \textbf{ASI06} & \textbf{Exc.Ag.} & \textbf{ASI10} \\
\midrule
\multirow{3}{*}{Clamp}
