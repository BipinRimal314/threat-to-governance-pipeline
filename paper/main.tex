...
% ============================================================================
\begin{abstract}
AI agent monitoring and insider threat detection share the same architecture: profile a baseline, flag deviations, encode assumptions about trust. We make this concrete across thirteen experiments. Using a Unified Behavioural Feature Schema (UBFS) that maps both employee activity logs and agent execution traces into a shared representation, we apply three anomaly detection models---Isolation Forest, LSTM Autoencoder, and Deep Clustering---across five domains. Cross-domain transfer is real but must be read carefully. An Isolation Forest trained on 329,000 insider-threat user-days scores 0.71 AUC-ROC on agent traces versus 0.73 in-domain---a 0.02 gap that falls within the seed-to-seed standard deviation ($\pm$0.047), so we frame this as \textbf{transfer within noise} rather than 97\% ``retention.'' This apparent robustness is partly an evaluation artifact: TRAIL is ${\sim}$97\% error-positive, so AUC-ROC rests on only a handful of negatives and AUC-PR ($\approx$0.98) largely tracks the base rate; tellingly, a detector trained on TRAIL itself scores \emph{lower} (0.58) than one transferred from CERT (0.71), which no genuine skill transfer would produce. We therefore report AUC-PR and recall at fixed low false-positive rates alongside AUC-ROC throughout. But the blind spots transfer too. Model distillation attacks span a detection spectrum from trivially caught (FOCUSED extraction: 0.99 AUC-ROC) to near-chance (HYDRA distributed: 0.54), and task decomposition---splitting a malicious objective into innocent subtasks---costs 5--6\% detection power for IF and DC ($p{=}0.031$, Wilcoxon signed-rank), while LSTM Autoencoder shows near-invariance ($-0.7\%$, $p{=}0.094$). Synthetic OWASP profiling identifies Tool Misuse (ASI02) as a blind spot (${\sim}$0.52 AUC-ROC), but \textbf{real-data validation on 500 ATBench trajectories} inverts that ranking exactly: ASI02 is the best-detected of six categories at 0.82--0.95, and Spearman $\rho$ between synthetic and real per-category rankings is $-1.000$ for all three models. The cause is visible in the generator---the ASI02 profile perturbs only privilege features while preserving behavioural structure, so it is undetectable by construction---and we report the divergence itself as a validity finding: synthetic perturbation profiling cannot locate blind spots, only restate the assumptions used to build them. Augmenting UBFS with eight semantic features (UBFS-28) improves detection by up to 13\% on real data. Distillation sensitivity analysis shows detection degrades monotonically with attack intensity for FOCUSED/BROAD/COT profiles, while HYDRA remains near-chance regardless of scale. Adversarial evasion testing reveals that mimicry attacks degrade detection by up to 25\%, with Memory \& Context Poisoning (ASI06) most vulnerable. Temporal window ablation finds that 10 spans are optimal for Isolation Forest on both TRAIL and ATBench. Mapping five MITRE ATLAS techniques~\cite{mitre2024atlas} into the UBFS confirms that the framework generalises beyond OWASP: all five techniques are strongly detectable ($>$0.93 AUC-ROC for IF/LSTM), introducing no new blind spots. The detection models port across domains. So do their biases, and so do their blind spots.
\end{abstract}
...
The results are stronger than expected in one direction, and more concerning than expected in another. Cross-domain transfer holds up---an insider threat model is not degraded on agent traces (a change within seed noise), and, on a balanced tool-calling benchmark where the metric is trustworthy, transfer to MCP traces measurably exceeds within-domain performance. But some blind spots transfer too---and telling those apart from the ones we invented turned out to be the harder problem. Distributed extraction is invisible to per-account monitoring because coordination is a property of the joint distribution and a per-entity detector only sees marginals; Anthropic's February 2026 disclosure of campaigns spread across 20,000+ accounts is independent corroboration~\cite{anthropic2026distillation}. Tool Misuse (ASI02) looked like a second instance of the same thing, and by the standard synthetic protocol it is: an agent using the right tools in the right order with escalated privileges, structurally undetectable for the same reason insider ``Scenario~2'' attacks evade detection in~\cite{rimal2026insider}. On real data it is the most detectable category we measure. The synthetic result was a restatement of the generator's own definition, and nothing inside the experiment could have shown that.
...
\begin{enumerate}
    ...
    \item \textbf{Transfer evidence}: Empirical demonstration that an Isolation Forest trained on 329,000 insider user-days is not degraded on agent traces (AUC-ROC: 0.731 $\to$ 0.711, a change within seed noise $\pm$0.047), with the caveat that TRAIL's ${\sim}$97\% positive prevalence makes its AUC-ROC an unstable estimand (Section~\ref{sec:transfer}).
    \item \textbf{OWASP detection matrix}: A per-category analysis under the standard synthetic protocol, showing three detection tiers---strong (ASI06: 0.975), moderate (Excessive Agency, ASI10: 0.70--0.86), and apparent blind spot (ASI02: 0.51--0.52). Contribution 9 shows why the third tier should not be believed.
    ...
    \item \textbf{Distillation detection spectrum}: Four distillation sub-profiles spanning AUC-ROC 0.54 (HYDRA distributed) to 0.99 (FOCUSED extraction), revealing a coordination blind spot: per-account behaviour in a distributed cluster is within normal bounds because coordination lives in the joint distribution a per-entity detector never observes.
    ...
    \item \textbf{UBFS-28 hybrid}: Semantic augmentation improves four categories on synthetic data and adds $+0.4$ to $+17.2\%$ on real data, with the largest gains on the weakest categories. It leaves synthetic ASI02 at ${\sim}$0.56, which contribution 9 shows is uninformative rather than a negative result.
    \item \textbf{A validity finding for synthetic OWASP profiling}: On 500 ATBench trajectories the synthetic per-category ranking is exactly inverted ($\rho = -1.000$, all three models), with the identified blind spot ASI02 turning out to be the best-detected category (0.52 synthetic vs.\ 0.82--0.95 real). The mechanism is that perturbation profiles measure their own definitions. We know of no way to detect this failure from inside the synthetic protocol, which makes real-data validation a precondition for blind-spot claims rather than a robustness check on them.
    ...
    \item \textbf{Adversarial evasion framework}: Three evasion strategies---feature clamping, gradual escalation, and mimicry---quantifying the robustness of UBFS-based detection. Mimicry is the most effective, degrading ASI06 detection by up to 25\%.
    ...
    \item \textbf{MITRE ATLAS mapping}: Five ATLAS adversarial ML techniques mapped to UBFS perturbation profiles, all expressible in the schema and all landing in the ``strong'' tier ($>$0.93 AUC-ROC for IF/LSTM). Being a synthetic mapping, this establishes expressiveness, not detectability.
\end{enumerate}
...
The anomaly detection models operate on UBFS vectors without knowing which domain they came from. This lossy compression trades domain-specific fidelity for cross-domain portability: CMU-CERT through UBFS achieves 0.731 AUC-ROC versus 0.799 with the full 24-feature pipeline from~\cite{rimal2026insider}.\footnote{We cite the reproducible value from the prior work's released harness (Isolation Forest, $0.799 \pm 0.017$); that paper's manuscript reports $0.807 \pm 0.012$ for the same configuration, and the two agree within their confidence intervals ($\Delta{=}0.008$, below either standard deviation). We standardise on the reproducible figure.} The 8.5\% drop quantifies the cost of abstraction. Two caveats apply. First, AUC-ROC is itself a limited headline metric here: the prior work shows that at the 5\% false-positive rate where security teams operate, Isolation Forest's operational recall is far below its AUC-ROC ranking, so the ``9\% cost of abstraction'' should be read as a ranking-quality cost, not an operational one. Whether this cost is acceptable depends on what you gain---and what we gain is the ability to transfer models across domains without retraining.
...
\textbf{Synthetic OWASP}: Anomalies injected into TRAIL traces to simulate four OWASP ASI categories (ASI01 Agent Goal Hijack, ASI02 Tool Misuse, ASI06 Memory \& Context Poisoning, ASI10 Rogue Agents) plus one profile with no ASI equivalent, Excessive Agency, which is drawn from the OWASP LLM Top 10. With anomaly ratio 0.5: ${\sim}$30 anomalous samples per category per seed. The injection method modifies UBFS features to match the expected structural signature of each attack type---e.g., ASI06 corrupts volume and output features; ASI02 modifies only privilege features while preserving all behavioural structure. That last profile is worth flagging here rather than in the results: it makes ASI02 undetectable to a behavioural model by construction, and Section~\ref{sec:real_owasp} shows the consequences.

\textbf{ATBench}~\cite{atbench2026}: 500 multi-turn agent trajectories with tool calls, binary safety labels (250 safe, 250 unsafe), and fine-grained risk categorisation mapped to OWASP ASI categories (ASI01 $n{=}61$, ASI02 $n{=}29$, ASI04 $n{=}29$, ASI06 $n{=}73$, ASI08 $n{=}29$, ASI10 $n{=}29$). After UBFS extraction: 500 samples. Used for MCP transfer validation (Experiment~7), hybrid detection (Experiment~8) and real-data OWASP validation (Experiment~12). The published dataset carries two configurations; every result here is computed on \texttt{ATBench500}, pinned in code, since the 1000-trajectory configuration is a different population with different per-category counts.
...
We evaluate three models, validated at up to 0.799 AUC-ROC on CMU-CERT in prior work~\cite{rimal2026insider}:
...
\textbf{Experiment~5} (Distillation detection): Evaluate detection of four distillation sub-profiles (FOCUSED, BROAD, COT, HYDRA) alongside the synthetic ASI02 profile as a floor reference, testing whether per-account behavioural monitoring can detect distributed extraction. [\RQ{4}]
...
\textbf{Experiment~8} (Hybrid detection): Augment UBFS-20 with 8 semantic features (UBFS-28) using sentence-transformer embeddings~\cite{reimers2019sentbert}, testing whether intent-aware features can close the \emph{synthetic} ASI02 blind spot. Also measures whether semantic features help or degrade cross-domain transfer. [\RQ{4}]
...
\midrule
\textit{Positive prevalence} & \textit{${\sim}$97\%} & \textit{${\sim}$50\%} & \textit{0.41\%} \\
\midrule
...
\end{tabular}
\par\smallskip
\footnotesize\textit{Prevalence varies by three orders of magnitude across datasets. On TRAIL (${\sim}$97\% positive) AUC-ROC is estimated against ${\sim}$5 negatives and AUC-PR tracks the base rate; interpret TRAIL scores accordingly.}
\end{table}
...
\textbf{CMU-CERT works through UBFS.} Isolation Forest achieves 0.731, lower than 0.799 with the full 24-feature pipeline~\cite{rimal2026insider}. The 8.5\% drop is the cost of the lossy UBFS compression---trading detection fidelity for cross-domain portability.
...
\textbf{The headline: CERT $\to$ TRAIL transfers within noise.} An Isolation Forest trained entirely on 329,000 insider threat user-days achieves 0.711 AUC-ROC on agent execution traces---0.019 below its within-domain performance of 0.731. We deliberately avoid the ``97\% retention'' framing: the 0.019 gap is smaller than the target's own seed-to-seed standard deviation ($\pm$0.047, $n{=}5$), so it is not distinguishable from noise. Two caveats sharpen this. First, TRAIL is ${\sim}$97\% error-positive, so its AUC-ROC rests on only a handful of negatives and is correspondingly unstable. Second, and decisively, an Isolation Forest \emph{trained on TRAIL itself} scores only 0.577 (Table~\ref{tab:baselines})---\emph{lower} than the 0.711 achieved by transfer from CERT. Genuine skill transfer cannot exceed in-domain training; this inversion indicates the TRAIL AUC-ROC is dominated by evaluation variance rather than by transferred detection skill. The honest reading is that CERT-trained detectors are \emph{not degraded} on TRAIL, not that detection power is measurably ``retained.'' Where transfer effects are large enough to trust, they are large: TRACE $\to$ CERT DC gains +0.223, an order of magnitude above within-domain variance.
...
$^{\dagger}$``Excessive Agency'' is from the OWASP LLM Top 10 and has no ASI equivalent; see below.}
\label{tab:owasp}
\centering
\footnotesize
...
\midrule
ASI06 Mem./Ctx.\ Pois. & $.920{\pm}.010$ & $\mathbf{.975{\pm}.006}$ & $.981{\pm}.013$ \\
Excessive Agency$^{\dagger}$ & $.858{\pm}.019$ & $.831{\pm}.014$ & $.852{\pm}.020$ \\
...
A note on the fifth row. ``Excessive Agency'' belongs to the OWASP \emph{LLM} Top 10, not the Agentic Top 10, and has no ASI equivalent: the nearest agentic risks are ASI10 Rogue Agents and ASI02 Tool Misuse, both generated separately here, so folding it into either would double-count. We keep the profile because it models a real and distinct behaviour---an agent taking far more action than its task requires---but we do not give it an ASI identifier.

\textbf{Tier 1: Detectable (ASI06).} Memory and context poisoning produces dramatic structural anomalies---corrupted outputs, unusual token volumes, broken execution patterns. All models detect it at 0.92 or above, with LSTM achieving 0.975 [CI: 0.970, 0.980].

\textbf{Tier 2: Moderate (Excessive Agency, ASI10, ASI01).} Over-broad action and rogue agent behaviour produce measurable structural deviations: more tool calls, broader scope, higher privilege. Detection ranges from 0.62 to 0.86, sufficient for triage but not reliable standalone.

\textbf{Tier 3: Blind spot (ASI02).} Tool Misuse achieves only 0.51--0.52 AUC-ROC---barely above chance. On the reading these synthetic results invite, the agent calls the right tools in the right order with the right sequencing and simply uses escalated privileges, so the behavioural \textit{structure} is identical to legitimate use---the AI equivalent of an insider using their real credentials to do something illegitimate.

That reading does not survive contact with real data, and the rest of this paper is in part an argument against it. Experiment~12 (Section~\ref{sec:real_owasp}) finds real ASI02 the \emph{best}-detected category of the six, at 0.82--0.95. The tier structure above is a property of how the anomalies were generated, not of the threat: the ASI02 profile is defined to modify only privilege features while preserving all behavioural structure, so a behavioural detector cannot see it \emph{by construction}. We report the tiers because they are what the standard synthetic protocol yields, and because the divergence from real data is itself the finding.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_owasp_matrix.png}
\caption{OWASP detection heatmap on \emph{synthetic} injections. ASI06 (Memory \& Context Poisoning) is reliably detected; ASI02 (Tool Misuse) sits at chance. The ASI02 result is an artifact of the injection profile, not a property of the threat---see Section~\ref{sec:real_owasp}. Six of the ten ASI categories were not testable with current synthetic generation.}
\label{fig:owasp}
\end{figure}

Six of the ten OWASP categories (ASI03 Identity \& Privilege Abuse, ASI04 Agentic Supply Chain Vulnerabilities, ASI05 Unexpected Code Execution, ASI07 Insecure Inter-Agent Communication, ASI08 Cascading Failures, ASI09 Human-Agent Trust Exploitation) could not be tested with the current synthetic generation framework. These categories require richer simulation of multi-agent interactions, network-level features, or supply chain dependencies that lie outside the UBFS's behavioural scope. Two of them---ASI04 and ASI08---are nonetheless reachable on real data, where ATBench supplies labelled instances, and Section~\ref{sec:real_owasp} reports both.

The parallel to insider threat detection is tempting, and worth stating precisely because it is where the synthetic protocol misleads. In~\cite{rimal2026insider}, Scenario~1 attacks (USB exfiltration with after-hours access) were detected at 69\%, while Scenario~2 attacks (quiet email/HTTP exfiltration) were detected at only 3\%. Read at face value the OWASP results mirror this: structurally loud attacks (ASI06, over-broad action) are detectable; structurally quiet ones (ASI02) are not. The ``boiling frog'' framing follows naturally.

It is the wrong lesson to draw from these numbers. The insider result is measured on recorded human activity; the ASI02 result is measured on perturbations we defined to be behaviourally invisible. The first is evidence about a threat, the second is a restatement of an assumption. Whether real agentic tool misuse is quiet is an empirical question the synthetic protocol cannot answer, and Section~\ref{sec:real_owasp} answers it in the negative.

The governance implication survives the correction, but in weaker form: behavioural monitoring has real structural ceilings---Section~\ref{sec:distillation} demonstrates one that does replicate---and synthetic profiling cannot be used to locate them.
...
\addlinespace
ASI02 (synthetic floor) & $0.497$ & $0.500$ & $0.491$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{HYDRA is a structural blind spot, and unlike ASI02 the argument for it does not rest on the profile.} Per-account behaviour in a distributed cluster (20,000+ accounts) falls within normal bounds; all three models achieve only ${\sim}$0.54.

The distinction from synthetic ASI02 matters, since both are our own constructions and Section~\ref{sec:real_owasp} shows what that can be worth. Synthetic ASI02 returns chance because the profile stipulates that no behavioural feature changes---the result is entailed by the definition, and tells us about the definition. HYDRA returns chance for a reason about the \emph{monitor}: a per-entity detector observes each account's marginal distribution and coordination is a property of the joint. Any per-entity model scoring accounts independently is blind to it however the individual accounts are simulated, and Experiment~13 confirms HYDRA stays at chance across every attack intensity while the other profiles degrade monotonically. That is an architectural limit rather than a stipulated one.

We nonetheless flag the asymmetry in evidence: the ASI02 claim was tested against real data and failed, while HYDRA has not been. No public benchmark we are aware of supplies labelled distributed-extraction campaigns at account granularity. The HYDRA result should be read as a well-motivated prediction with synthetic support, not as a validated finding.
...
\textbf{TRAIL $\to$ ATBench IF exceeds within-domain performance.} At 0.878 AUC-ROC, the transferred Isolation Forest outperforms the within-domain IF (0.838) by 4.8\%---a 104.8\% retention rate. Unlike the CERT $\to$ TRAIL headline, this gain is statistically robust: the 0.041 improvement is roughly an order of magnitude larger than the seed-to-seed standard deviation ($\pm$0.002), and it is measured on a balanced benchmark (ATBench AUC-PR 0.77--0.83) where AUC-ROC is a trustworthy estimand rather than resting on a handful of negatives. This extends the Experiment~2 finding: not only does anomaly knowledge transfer across the insider/agent boundary, it transfers across \textit{agent benchmark boundaries}. The mechanism is the same as TRACE $\to$ CERT in Experiment~2---the source domain's training data provides a richer characterisation of ``normal'' than the target domain alone.
...
\midrule
ASI02 Tool Misuse & $0.871$ & 29 \\
ASI06 Memory \& Context Poisoning & $0.869$ & 73 \\
ASI04 Agentic Supply Chain Vuln. & $0.848$ & 29 \\
ASI01 Goal Hijack & $0.843$ & 61 \\
ASI08 Cascading Failures & $0.776$ & 29 \\
ASI10 Rogue Agents & $0.764$ & 29 \\
\bottomrule
\end{tabular}
\end{table}

Notably, ASI02 is the \emph{best}-detected category on ATBench at 0.871---the same category that sits at 0.520 under synthetic injection in Experiment~3. The gap is not a matter of degree. The synthetic profile perturbs only privilege features by construction, so it can only ever be invisible to a behavioural detector; real tool misuse in ATBench carries the structural signature the profile stipulates away. Section~\ref{sec:real_owasp} takes this up as the paper's methodological result.
...
Excessive Agency$^{\dagger}$ & .814 & $\mathbf{.951}$ & +.138 & LSTM \\
ASI06 Mem.\ \& Ctx.\ Poison. & $\mathbf{.989}$ & .976 & --.013 & DC \\
\addlinespace
ASI02 Tool Misuse & .537 & .559 & +.023 & DC \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^{\dagger}$OWASP LLM Top 10; no ASI code.} \\
\end{tabular}
\end{table}

\textbf{Semantic features transform detection for four categories.} ASI01 jumps from 0.63 to 0.94 (LSTM, $+0.31$); ASI10 from 0.67 to 0.94 ($+0.27$); Excessive Agency from 0.81 to 0.95 ($+0.14$). Intent consistency and goal drift scores expose the semantic mismatch that behavioural features miss. ASI06, already near-perfect, shows no improvement. All values in Table~\ref{tab:hybrid} use a single model per row (the best UBFS-28 performer) to ensure the delta is a fair within-model comparison.

\textbf{Synthetic ASI02 does not move, and this is uninformative.} Adding 8 semantic dimensions designed to capture intent leaves Tool Misuse at ${\sim}$0.54 to ${\sim}$0.56 (best model DC: 0.537 $\to$ 0.559). We originally read this as confirmation that ASI02 is a structural limitation of monitoring-based detection. It is not evidence for that claim. The synthetic ASI02 profile modifies only privilege features; UBFS-28's additional dimensions are semantic, not authorisation-derived; so no semantic feature could have moved this number regardless of what real tool misuse looks like. The experiment is a check that the semantic features are not accidentally sensitive to the privilege dimension, and it passes. On real data, where the perturbation is not ours to define, UBFS-28 improves ASI02 by 2.5--9.5\% over an already-strong baseline (Section~\ref{sec:real_owasp}).
...
\begin{table}[htbp]
\caption{Adversarial Evasion: AUC-ROC Drop (\%) by Strategy. EA${=}$Excessive Agency (not an ASI category).}
\label{tab:evasion}
\centering
\small
\begin{tabular}{@{}llrrrrr@{}}
\toprule
\textbf{Strategy} & \textbf{Model} & \textbf{ASI01} & \textbf{ASI02} & \textbf{ASI06} & \textbf{EA} & \textbf{ASI10} \\
\midrule
\multirow{3}{*}{Clamp}
...
Three findings emerge from Table~\ref{tab:evasion}. \textbf{ASI06 is most vulnerable.} Memory and context poisoning relies on volume spikes that are easily capped; clamping drops LSTM detection by 21\%, and mimicry drops it by 25\%. This category's high baseline AUC-ROC (0.976) is artificially inflated by the same feature visibility that makes it easy to evade.

\textbf{Synthetic ASI02 is unaffected by evasion, trivially.} All three strategies produce $<$2\% change. This is not a robustness property: mimicry is defined to replace every feature except the privilege dimension, and the synthetic ASI02 profile perturbs only the privilege dimension, so mimicry and the profile do not overlap. A number that cannot move tells us nothing about the category, and we do not draw a conclusion from it here.

\textbf{Gradual escalation reveals a sharp threshold.} Only ASI06 triggers detection before full escalation ($\alpha = 0.64$ for IF, $\alpha = 0.92$ for LSTM). All other categories require $\alpha = 1.0$---the complete anomalous profile---before crossing the detection threshold. This means behavioural monitoring is binary, not gradual: either the anomaly is large enough to detect, or it is invisible. There is no ``early warning'' for subtle attacks.
...
\textbf{No new blind spots.} The cross-taxonomy comparison reveals that all five ATLAS techniques land in the ``strong'' detection tier ($>$0.80 for IF), because ATLAS techniques are volume-intensive by nature and therefore structurally visible. We note that this experiment shares the limitation of Experiment~3: ATLAS techniques are mapped into UBFS as perturbation profiles we specify, so ``strongly detectable'' here means the profiles are loud as written. The claim this supports is the narrow one---mapping a second taxonomy onto the UBFS introduces no category the schema cannot express---and not a claim about how detectable real ATLAS-technique traffic would be.
...
Experiment~12 tests whether the synthetic OWASP detection tiers from Experiment~3 hold on real data. They do not. This is the paper's methodological result, and we state it before the table: on every category the two experiments share, the synthetic protocol ranks detectability in exactly the reverse of the real order.

The mechanism is circularity. A perturbation profile specifies which UBFS dimensions an attack modifies; the detector then reports how visible those modifications are. What comes out is a measurement of the profile, not of the threat. Where a profile happens to encode a real attack's structure the two coincide and the circularity is invisible. Where it does not---as with ASI02, defined to modify only privilege features while preserving all behavioural structure---the experiment cannot return anything but ``undetectable,'' whatever real tool misuse looks like. ATBench provides 500 real agent trajectories with ground-truth risk labels mapped to OWASP ASI categories, which breaks the circle.
...
\toprule
\textbf{Model} & \textbf{Feat.} & \textbf{ASI01} & \textbf{ASI02} & \textbf{ASI04} & \textbf{ASI06} & \textbf{ASI08} & \textbf{ASI10} \\
\midrule
\multirow{2}{*}{IF}
 & -20 & .843 & .871 & .848 & .869 & .776 & .764 \\
 & -28 & $\mathbf{.943}$ & $\mathbf{.954}$ & $\mathbf{.927}$ & $\mathbf{.939}$ & $\mathbf{.909}$ & $\mathbf{.877}$ \\
\addlinespace
\multirow{2}{*}{LSTM}
 & -20 & .785 & .823 & .801 & .798 & .733 & .714 \\
 & -28 & .857 & .876 & .845 & .825 & .819 & .823 \\
\addlinespace
\multirow{2}{*}{DC}
 & -20 & .894 & .910 & .897 & .906 & .858 & .850 \\
 & -28 & .926 & .933 & .909 & .910 & .895 & .873 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The ASI02 ``blind spot'' does not exist on real data.} It is not merely weaker than the synthetic result suggests: ASI02 is the \emph{best}-detected of the six categories, under all three models and both feature sets---0.871, 0.823 and 0.910 at UBFS-20, rising to 0.954, 0.876 and 0.933 at UBFS-28. Experiment~3 places the same category at 0.520, at the bottom of its ranking and barely above chance. The synthetic protocol does not underestimate ASI02 by a margin; it identifies as the hardest category the one that is in fact the easiest.

\textbf{The synthetic ranking is exactly inverted.} Across the three categories the two experiments share (ASI01, ASI02, ASI10), Spearman $\rho = -1.000$ for all three models: every category the synthetic protocol ranks most detectable, real data ranks least. We report this as an effect and not as a significance test. With $n{=}3$ categories there are only $3!$ orderings, so the smallest two-sided $p$ an exact permutation test can return is $1/3$; the observed $p = 0.333$ \emph{is} that floor, and no result at this $n$ could be significant at any conventional level.\footnote{A $t$-approximation reports $p < 10^{-15}$ here, because at $\rho = \pm 1$ its denominator is zero. We compute the exact permutation $p$ throughout, and report the attainable floor alongside it, so that a degenerate statistic cannot be mistaken for a strong one.} A rank correlation over three points is weak evidence however it comes out. The result is stated because its direction is consistent across three independent models and because the mechanism is known in advance from the generator's definition---not because the correlation is significant.

\textbf{UBFS-28 improves real-data detection, most where UBFS-20 is weakest.} Semantic features add $+8.0$ to $+17.2\%$ for Isolation Forest, $+3.4$ to $+15.2\%$ for LSTM, and $+0.4$ to $+4.3\%$ for Deep Clustering. The largest gains fall on the two weakest categories, ASI08 (IF: $+17.2\%$) and ASI10 (LSTM: $+15.2\%$), which is the desirable direction. Unlike the synthetic results of Experiment~8, where UBFS-28 left ASI02 near chance, on real data it lifts an already-strong ASI02 by a further $2.5$--$9.5\%$.

The conclusion is not that synthetic profiling is a useful lower bound. On this evidence it is not a bound at all: it produced a confident, quotable, exactly wrong ordering, and nothing internal to the experiment could have revealed that. Synthetic profiling can demonstrate that a detector responds to a specified perturbation. It cannot be used to locate blind spots, which is the use it is most often put to---including, in an earlier version of this work, by us.
...
\textbf{2. Blind spots must be located empirically, not by construction.} It is tempting to conclude that agentic tool misuse evades detection for the same reason Scenario~2 insiders do in~\cite{rimal2026insider}---the behaviour is structurally legitimate---and an earlier version of this paper drew exactly that parallel. Experiment~12 shows it was an artifact of how we generated the data. The blind spots that survive contact with evidence are the architectural ones: per-entity monitoring cannot see coordination (HYDRA), and behavioural features cannot see semantic wrongness (TRACE). A governance framework should treat any blind spot established only on synthetic perturbations as unverified, including the ones in this paper that we have not yet been able to test.
...
Transfer succeeds when the source and target domains share structural dimensions: temporal patterns, frequency distributions, scope metrics. The UBFS captures these shared dimensions, enabling Isolation Forest---which learns global isolation in feature space---to transfer without measurable degradation (CERT $\to$ TRAIL: 0.731 $\to$ 0.711, within seed noise) and, on the balanced ATBench benchmark, to exceed within-domain performance.
...
Transfer fails when the anomaly is semantic rather than structural. Reward hacking (TRACE) produces traces that are structurally normal but semantically wrong, and no amount of training data from any domain will detect this through behavioural features alone. This holds on a real dataset we did not construct, which is why we retain it.

We previously placed Tool Misuse alongside TRACE as a second instance of the same failure, on the reasoning that the agent invokes the right tools in the right order and only the \textit{why} is anomalous. Experiment~12 withdraws that claim: real tool misuse in ATBench is the most detectable of the six categories, because misuse in practice comes with the structural traces the argument assumed away---atypical tool sequences, unusual parameter combinations, targets outside the session's established scope. The \textit{why} may be what makes it misuse, but it is not the only thing that changes.
...
\textbf{Synthetic OWASP.} The OWASP detection matrix (Experiment~3) uses synthetically injected anomalies with circular methodology, and Experiment~12 shows the resulting ranking is inverted relative to real data ($\rho = -1.000$ on the three shared categories, all three models). We report Experiment~3 because it is the protocol the literature uses and because the divergence is the finding, not because its tiers should be believed. Only three categories are shared between the two experiments, which is too few for the rank correlation to be significant at any $n{=}3$; the claim rests on the size and consistency of the per-category gaps, not on the correlation. Six of the ten ASI categories remain untested synthetically and four untested on real data.
...
\textbf{Semantic feature scope.} The UBFS-28 semantic features (Experiment~8) are computed from sentence-transformer embeddings of tool parameters and conversation context. They capture textual intent signals but not authorisation context---no feature in either schema encodes who granted an instruction or whether the grant was valid. This bounds what the schema can express; it is not, on the evidence of Experiment~12, what makes real tool misuse hard to catch.

\subsection{Future Work}

Three directions follow from the experimental results.

\textbf{Real-data validation for the remaining categories.} The single most useful extension of this work is not a better model. Four ASI categories have no real-data result here and six have no synthetic one, and Experiment~12 shows that a synthetic estimate can be confidently and exactly wrong. Benchmarks with labelled instances of ASI03, ASI05, ASI07 and ASI09, and any dataset supporting account-level distributed extraction so that HYDRA can be tested rather than predicted, would do more for the reliability of this literature than further architecture search.
...
\textbf{Adversarial-robust features.} Experiment~9 shows that ASI06's high detection rate is brittle---mimicry evasion drops LSTM from 0.976 to 0.731 AUC-ROC. Future work should develop features robust to nearest-neighbour mimicry, such as higher-order interaction terms or temporal signatures that cannot be individually replaced.
...
\textbf{First, the transfer holds up---and generalises.} An Isolation Forest trained on insider threat data is not degraded on agent traces (Experiment~2; the 0.019 change is within seed noise, and TRAIL's ${\sim}$97\% prevalence makes its AUC-ROC an unstable estimand), and---on a balanced benchmark where the metric is trustworthy---transfer to MCP tool-calling traces measurably exceeds within-domain performance at 104.8\% (Experiment~7, a gain ${\sim}$18$\times$ the seed std). The UBFS bridge enables cross-domain anomaly detection across multiple agent benchmark boundaries, with the strongest evidence coming from the balanced-prevalence settings.

\textbf{Second, one of our two ``structural blind spots'' was an artifact of our own generator.} Experiment~3 identifies Tool Misuse (ASI02) as a blind spot at ${\sim}$0.52 AUC-ROC, and Experiment~8's semantic augmentation leaves it at ${\sim}$0.56. On 500 real ATBench trajectories it is the \emph{best}-detected of six categories, at 0.82--0.95. The synthetic ranking is not merely miscalibrated but exactly inverted ($\rho = -1.000$ on the shared categories, consistently across all three models). The cause is visible in the generator: the ASI02 profile modifies only privilege features while preserving all behavioural structure, which makes it undetectable to a behavioural model by definition rather than by finding.

\textbf{Third, the surviving blind spot is architectural, not behavioural.} Distributed extraction (HYDRA) stays at ${\sim}$0.54 because a per-entity detector scores each account's marginal distribution while coordination lives in the joint---a limit of the monitoring architecture that no feature added to a per-account model can lift. It is corroborated by the Anthropic GTG-1002 disclosure, though not yet by a labelled benchmark. The distinction from ASI02 is the paper's practical lesson: a blind spot argued from the structure of the monitor survived, and a blind spot measured from a perturbation we defined did not. Synthetic profiling generates hypotheses. It does not locate blind spots, and we would not have discovered that about our own results without a real benchmark to check them against.
...
\textbf{Fifth, high detection rates are brittle.} Experiment~9 shows that mimicry evasion---copying a normal neighbour's features while preserving the attack payload---degrades ASI06 detection by up to 25\%. The categories that \textit{appear} most detectable are the ones most vulnerable to informed adversaries: a detector that leans on a few loud features can have those features replaced.
