...
\begin{enumerate}
    ...
    \item \textbf{Real-data validation}: 500 ATBench trajectories reveal that synthetic OWASP profiling dramatically underestimates real detection performance (ASI02: 0.52 synthetic vs.\ 0.86--0.94 real), and UBFS-28 improves all categories by 5--13\% on real data.
...
\addlinespace
ASI02 Tool Misuse & $.520{\pm}.026$ & $.510{\pm}.019$ & $.524{\pm}.038$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Tier 1: Detectable (ASI05).} Memory poisoning produces dramatic structural anomalies---corrupted outputs, unusual token volumes, broken execution patterns. All models detect it above 0.92, with LSTM achieving 0.975 [CI: 0.970, 0.980].
...
\textbf{The ASI02 blind spot is a synthetic artifact.} On real ATBench data, ASI02 achieves 0.86--0.94 AUC-ROC across all models---far above the 0.51--0.52 seen with synthetic injection in Experiment~3. Real-world tool misuse produces structurally visible anomalies (unusual parameter combinations, atypical tool sequences) that the circular synthetic profiles---designed to modify only the privilege dimension---cannot capture.

\textbf{UBFS-28 consistently improves real-data detection.} Semantic features add 5--13\% across all categories and models, with the largest gains on ASI04 (IF: $+$13.3\%) and ASI10 (IF: $+$11.3\%). Unlike the synthetic results of Experiment~8, where UBFS-28 failed to close ASI02, on real data UBFS-28 improves ASI02 by 4.8--8.1\% depending on model.
...
\textbf{Third, synthetic blind spots require real-data validation.} Experiment~3's synthetic profiling identifies ASI02 as a blind spot (${\sim}$0.52 AUC-ROC), and Experiment~8's semantic augmentation leaves it at ${\sim}$0.56. But Experiment~12 reveals this is partially a circular methodology artifact: real ATBench data yields 0.86--0.94 on ASI02, and UBFS-28 adds 5--13\% across all categories on real data. The lesson: synthetic profiling provides useful lower bounds but not absolute calibration (Spearman $\rho{=}0.20$ between synthetic and real rankings).
